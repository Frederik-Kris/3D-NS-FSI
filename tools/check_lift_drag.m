...
C_D_list(end+1)=C_D;
fprintf("Result %i: C_D = %f\n", vtkNum, C_D);

end % for

set(groot,'defaultLineLineWidth',2.0);
save('./C_D_list.mat', 'C_D_list');

figure(1);
plotTimes = [linspace(100, 4000, 40), linspace(4040, 4040+(nFiles-40-1)*40, nFiles-40)];
plot(plotTimes, C_D_list);
hold on;
plot([plotTimes(1), plotTimes(end)], mean(C_D_list)*[1,1]);
plot([plotTimes(round(end/2+1)), plotTimes(end)], mean(C_D_list(round(end/2+1):end))*[1,1]);
plot([plotTimes(round(end*3/4+1)), plotTimes(end)], mean(C_D_list(round(end*3/4+1):end))*[1,1]);
title("Drag coefficient");
legend("Samples", "Average all", "Average last half", "Average last quarter");
